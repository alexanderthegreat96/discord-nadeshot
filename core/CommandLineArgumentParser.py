...
class CommandLineArgumentParser:
        ...
        if len(args) > 1:
            ...
            if len(keys) < 1:
                ...
                for i in range(0, len(args), 2):
                    ...
                    result_dict[key] = (
                        self.wrap_string_with_underscore(value)
                        if value is not None
                        else None
                    )
                    generated.append(result_dict)
    ...
    def validate_arguments(
        self, command_input=None, arguments=None, command_syntax=None
    ):
        errors = []
        argument_assoc = []
        status = True
        ...
        if command_input and arguments:
            ...
            for argument_key, values in arguments.items():
                if "required" in values and values["required"]:
                    if argument_key in input_to_array:
                        ...
                        if self.checkIfIndexIsOutOfRange(
                            input_to_array, argument_value_index
                        ):
                            if "hasValue" in values:
                                ...
                                if has_value and argument_value:
                                    if (
                                        has_value
                                        and "minLength" not in values
                                        and "maxLength" not in values
                                        and argument_value is None
                                        and argument_value == " "
                                    ):
                                        status = False
                                        errors.append(
                                            f"Argument {argument_key} MUST have a value."
                                        )
                                    else:
                                        if "minLength" in values:
                                            ...
                                            if (
                                                ...
                                                and len(argument_value) < min_length
                                            ):
                                                status = False
                                                errors.append(
                                                    f"Argument {argument_key} value is TOO SHORT. Minimum length is {min_length}"
                                                )
                                        if "maxLength" in values:
                                            max_length = values.get("maxLength")
                                            if (
                                                max_length is not None
                                                and len(argument_value) > max_length
                                            ):
                                                status = False
                                                errors.append(
                                                    f"Argument {argument_key} value is TOO LONG. Maximum length is {max_length}"
                                                )
                                            ...
                                            if type == "boolean":
                                                if argument_value not in [
                                                    ...
                                                    argument_assoc.append(
                                                        {
                                                            argument_key: self.converToBoolean(
                                                                argument_value
                                                            )
                                                        }
                                                    )
                                                ...
                                                if "accepts" in values and len(
                                                    values["accepts"]
                                                ):
                                                    if (
                                                        argument_value
                                                        not in values["accepts"]
                                                    ):
                                                        status = False
                                                        errors.append(
                                                            f"Argument {argument_key} must contain one of the values: "
                                                            + str(values["accepts"])
                                                        )
                                                    else:
                                                        argument_assoc.append(
                                                ...
                                                convertToInt = self.convertToInteger(
                                                    argument_value
                                                )
                                                if convertToInt:
                                                    if not self.isInteger(convertToInt):
                                                        status = False
                                                        errors.append(
                                                            f"Argument {argument_key} must be an INTEGER"
                                                        )
                                                    else:
                                                        argument_assoc.append(
                                                            {argument_key: convertToInt}
                                                        )
                                                else:
                                                    status = False
                                                    errors.append(
                                                ...
                                                convertToFloat = self.converToFloat(
                                                    argument_value
                                                )
                                                if convertToFloat:
                                                    if not self.isFloat(convertToFloat):
                                                        status = False
                                                        errors.append(
                                                            f"Argument {argument_key} must be a FLOAT"
                                                        )
                                                    else:
                                                        argument_assoc.append(
                                                            {
                                                                argument_key: convertToFloat
                                                            }
                                                        )
                                                ...
                                                argument_assoc.append(
                                                    {
                                                        argument_key: self.wrap_string_with_underscore(
                                                            argument_value
                                                        )
                                                    }
                                                )
                            ...
                            status = False
                            errors.append(
                                f"Argument {argument_key} is REQUIRED and must have a VALUE."
                            )

                    else:
                        status = False
                        errors.append(
                            f"Argument {argument_key} is REQUIRED and must be provided."
                        )
            ...
            difference = self.calculate_symmetric_difference(
                syntax_to_array, input_to_array
            )
            last_argument = None
            ...
            if difference:
                ...
                if arguments:
                    ...
                    for argument, argument_details in arguments.items():
                        if (
                            "hasValue" in argument_details
                            and argument_details["hasValue"] == True
                        ):
            ...
            if last_argument in arguments:
                ...
                if "hasValue" in arg_data and arg_data["hasValue"] == True:
                    ...
                    if self.checkIfIndexIsOutOfRange(
                        ...
                        if "hasValue" in arg_data:
                            ...
                            if has_value and argument_value:
                                ...
                                if (
                                    ...
                                    status = False
                                    errors.append(
                                        f"Argument {last_argument} MUST have a value."
                                    )
                                else:
                                ...
                                if "type" in arg_data:
                                    ...
                                    if type is not None:
                                        if type == "boolean":
                                            ...
                                            if "accepts" in arg_data and len(
                                                arg_data["accepts"]
                                            ):
                                                if (
                                                    argument_value
                                                    not in arg_data["accepts"]
                                                ):
                                                    status = False
                                                    errors.append(
                                                        f"Argument {last_argument} is provided and must contain one of the values: "
                                                        + str(arg_data["accepts"])
                                                    )
                                            ...
                                            if convertToInt:
                                                if not self.isInteger(convertToInt):
                                                    status = False
                                                    errors.append(
                                                        f"Argument {last_argument} must be an INTEGER"
                                                    )
                                            ...
                                            if convertToFloat:
                                                if not self.isFloat(convertToFloat):
                                                    status = False
                                                    errors.append(
                                                        f"Argument {last_argument} must be a FLOAT"
                                                    )
                        ...
                        status = False
                        errors.append(
                            f"Argument {last_argument} is provided and must have a VALUE."
                        )
            ...
            if not argument_assoc and difference:
                argument_assoc = self.generateArgumentAssociationUnique(
                    difference, arguments.keys()
                )
    ...
    def validate_command_input(self, command_input, argument=None):
        ...
        if command_input and argument:
            ...
            if argument in input_to_array:
                ...
                if not self.checkIfIndexIsOutOfRange(
                    input_to_array, argument_value_index
                ):
    ...
    def seekCommands(self, commands):
        ...
        if matching_command:
            ...
            if not has_value:
                status, errors, args = self.validate_arguments(
                ...
                status, errors, args = self.validate_command_input(
                    self.input, command_arg
                )
        ...
        if commands:
            for command in commands:
                if "commands" in commands[command] and commands[command]["commands"]:
                    nested_data = self.pull_commands_in_the_same_array(
        ...
        if arguments:
            for argument_key, argument_value in arguments.items():
                if "required" not in argument_value:
                    errors.append(
                        "Argument: [" + argument_key + "] is missing ['required'] key."
                    )
                    status = False
                else:
                    if argument_value["required"] not in ["true", "false", True, False]:
                        errors.append(
                            "Argument: ["
                            + argument_key
                            + "] key: ['required'] must be a boolean."
                        )
                        status = False

                if "hasValue" not in argument_value:
                    errors.append(
                        "Argument: [" + argument_key + "] is missing ['hasValue'] key."
                    )
                    status = False
                else:
                    if argument_value["hasValue"] not in ["true", "false", True, False]:
                        errors.append(
                            "Argument: ["
                            + argument_key
                            + "] key: ['hasValue'] must be a boolean."
                        )
                        status = False
                ...
                if "minLength" in argument_value:
                    if not self.isInteger(argument_value["minLength"]):
                        errors.append(
                            "Argument: ["
                            + argument_key
                            + "] key: ['minLength'] must be an integer."
                        )
                        status = False
                ...
                if "maxLength" in argument_value:
                    if not self.isInteger(argument_value["maxLength"]):
                        errors.append(
                            "Argument: ["
                            + argument_key
                            + "] key: ['maxLength'] must be an integer."
                        )
                        status = False
                ...
                if "type" in argument_value:
                    if argument_value["type"] not in [
                        ...
                        errors.append(
                            "Argument: ["
                            + argument_key
                            + "] key ['type'] must contain one of the "
        ...
        if commands:
            for command in commands:
                if command["command"] is None or command["command"] == "":
                    errors.append(
                        "['syntax'] in " + command["name"] + " was not declared"
                    )
                    status = False
                ...
                if command["desc"] is None or command["desc"] == "":
                    errors.append(
                        "['description'] in "
                        + command["name"]
                        + " was not declared or it is empty"
                    )
                    status = False

                if "authorization" not in command:
                    errors.append(
                        "['authorization'] in " + command["name"] + " was not declared."
                    )
                    status = False
                ...
                if (
                    "hasValue" not in command
                    or command["hasValue"] is None
                    or command["hasValue"] == ""
                ):
                    errors.append(
                        "['hasValue'] in "
                    ...
                    if command["hasValue"] not in [True, False, "true", "false"]:
                        errors.append(
                            "['hasValue'] in " + command["name"] + " "
                            "has to be set to either 'true' or 'false'"
                        )
                        status = False
                if command["arguments"] is not None:
                    arg_errors, arg_status = self.check_args_integrity(
                        command["arguments"]
                    )
                    if not arg_status:
    ...
    def build_command_helper(self, commands=None, data=None):
        ...
        if commands:
            for command in commands:
                if "commands" in commands[command] and commands[command]["commands"]:
                    nested_data = self.build_command_helper(
                        commands[command]["commands"]
                    )
                    data.extend(nested_data)
                else:
        ...
        if check_status:
            ...
            if commands:
                ...
                if results:
                    ...
                    return {
                        "status": False,
                        "error": "No command found for your input. Check the command line helper.",
                    }
            else:
