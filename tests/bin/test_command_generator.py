...
class TestGenerateCommand:
    ...
    def test_manipulate_commands_json_root_command_with_dash_fails(
        self, mock_from_root
    ):
    ...
    def test_manipulate_commands_json_invalid_json(self, mock_from_root):
        ...
        with patch("CommandGenerator.path.exists", return_value=True):
            with patch("builtins.open", mock_open(read_data="invalid json")):
                ...
                assert result["status"] is False
                assert (
                    "error" in result or "decoding" in result.get("error", "").lower()
                )
        ...
        with patch("CommandGenerator.path.exists", return_value=True):
            with patch("builtins.open", mock_open(read_data=json.dumps(initial_data))):
                with patch.object(
                    GenerateCommand,
                    "write_py_files",
                    return_value={"status": True, "message": "Created"},
                ):
