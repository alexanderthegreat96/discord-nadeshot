...
class TestBot:
    ...
    def test_parsed_command_list_success(self):
        ...
        mock_commands = {
            "commands": {"test_cmd": {"syntax": "/test", "description": "Test command"}}
        }
                    ...
                    with patch("core.Bot.Logger"):
                        with patch("core.Bot.Cache"):
                            with patch("core.Bot.MultiBotHandler"):
                                with patch(
                                    "builtins.open",
                                    mock_open(read_data=json.dumps(mock_commands)),
                                ):
                                    bot = Bot()
                    ...
                    with patch("core.Bot.Logger"):
                        with patch("core.Bot.Cache"):
                            with patch("core.Bot.MultiBotHandler"):
                                with patch(
                                    "builtins.open",
                                    mock_open(read_data=json.dumps(mock_tasks)),
                                ):
                                    bot = Bot()
    ...
    def test_staff_groups_success(self):
        ...
        mock_groups = {"groups": {"admin": ["user1", "user2"], "mod": ["user3"]}}
                    ...
                    with patch("core.Bot.Logger"):
                        with patch("core.Bot.Cache"):
                            with patch("core.Bot.MultiBotHandler"):
                                with patch(
                                    "builtins.open",
                                    mock_open(read_data=json.dumps(mock_groups)),
                                ):
                                    bot = Bot()
